    ...
    public class GameEventManager : MonoBehaviour
    {
        ...
        public static GameEventManager Instance
        {
            get
            {
                ...
                s_instance = FindFirstObjectByType<GameEventManager>();
        ...
        private void Awake()
        {
            ...
            {
                DDebug.Log(
                    $"There cannot be two {typeof(GameEventManager)}' active at the same time. Destroying this one!");
                Destroy(gameObject);
                return;
            }
        ...
        public static void ProcessGameEvent(GameEventMessage message, bool debug = false)
        {
            ...
            if (Instance.DebugComponent || debug && !message.IsSystemEvent) DDebug.Log(
                $"Received '{message.EventName}' game event.", Instance);
