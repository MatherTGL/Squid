...
namespace Doozy.Engine.Progress
{
    ...
    public class Progressor : MonoBehaviour
    {
        ...
        public bool CanBeOutOfRangeValue;
...
//            m_previousValue = Value;
            ...
            if (!enabled||instantUpdate || !AnimateValue)
            {
                Value = value;
                return;
            }
            ...
            if (!m_tweenInitialized)
                ...
                //m_animationSequence.Append(m_tween).Play();
        }
        ...
        public void UpdateProgress()
        {
            if (DebugComponent) DDebug.Log(
                $"[{name}] Value: {Value} / Progress: {Progress} / Inverse Progress: {InverseProgress}", this);
        ...
        public void SetMin(float value)
        {
            ...
            if (!CanBeOutOfRangeValue && Value < m_minValue) SetValue(m_minValue);
            UpdateProgress();
        }
        ...
        public void SetMax(float value)
        {
            ...
            if (!CanBeOutOfRangeValue && Value > m_maxValue) SetValue(m_maxValue);
            UpdateProgress();
        }
        ...
        {
            if(!CanBeOutOfRangeValue) value = Mathf.Clamp(value, MinValue, m_maxValue);
