...
namespace Doozy.Engine.SceneManagement
{
    ...
    public class SceneLoader : MonoBehaviour
    {
        ...
        public AsyncOperationAdapter CurrentAsyncOperation { get; private set; }
...
//            Debug.Log("CurrentAsyncOperation == null? " + (CurrentAsyncOperation == null));
            ...
            if (DebugComponent && !m_activatingScene) DDebug.Log(
                $"[{name}] Load progress: {Mathf.Round(Progress * 100)}%", this);
        ...
        public void LoadSceneAsync()
        {
            ...
            switch (GetSceneBy)
            {
                ...
                case GetSceneBy.BuildIndex:
                    ...
                    break;
                case GetSceneBy.Addressable:
                    LoadSceneAsyncAdressables(SceneName, LoadSceneMode);
                    break;
            }
        }
        ...
        public Progressor LoadSceneAsync(int sceneBuildIndex, LoadSceneMode mode)
        {
            ...
            CurrentAsyncOperation = new AsyncOperationAdapter(SceneManager.LoadSceneAsync(sceneBuildIndex, mode));
            StartSceneLoad();
            return Progressor;
        }
        ...
        public Progressor LoadSceneAsync(string sceneName, LoadSceneMode mode)
        {
            ResetProgress();
            LoadBehavior.OnLoadScene.Invoke(gameObject);
            CurrentAsyncOperation = new AsyncOperationAdapter(SceneManager.LoadSceneAsync(sceneName, mode));
            StartSceneLoad();
            return Progressor;
        }
        
        public Progressor LoadSceneAsyncAdressables(string sceneName, LoadSceneMode mode)
        {
            ResetProgress();
            LoadBehavior.OnLoadScene.Invoke(gameObject);
            CurrentAsyncOperation = new AsyncOperationAdapter(sceneName, mode);
            StartSceneLoad();
            return Progressor;
        }
...
//                if (CurrentAsyncOperation == null) yield break;
                ...
                if (DebugComponent && !activatingScene) DDebug.Log(
                    $"[{name}] Load progress: {Mathf.Round(Progress * 100)}%", this);
...
//                    CurrentAsyncOperation = null;
        ...
        {
            var loader = new GameObject(nameof(SceneLoader)).AddComponent<SceneLoader>();
