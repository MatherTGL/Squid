    ...
    public class SceneDirector : MonoBehaviour
    {
        ...
        public static SceneDirector Instance
        {
            get
            {
                ...
                s_instance = FindFirstObjectByType<SceneDirector>();
        ...
        private void Awake()
        {
            ...
            {
                DDebug.Log(
                    $"There cannot be two {typeof(SceneDirector)}' active at the same time. Destroying this one!");
                Destroy(gameObject);
                return;
            }
        ...
        {
            if (Instance.DebugComponent) DDebug.Log(
                $"LoadSceneAsync - sceneBuildIndex: {sceneBuildIndex} / loadSceneMode: {loadSceneMode} / has Progressor: {(progressor == null ? "No" : "Yes")}", Instance);
        ...
        {
            if (Instance.DebugComponent) DDebug.Log(
                $"LoadSceneAsync - sceneName: {sceneName} / loadSceneMode: {loadSceneMode} / has Progressor: {(progressor == null ? "No" : "Yes")}", Instance);
            SceneLoader loader = SceneLoader.GetLoader();
        ...
        public static AsyncOperationHandle<SceneInstance> UnloadSceneAsyncAdressables(string sceneName)
        {
            if (Instance.DebugComponent) DDebug.Log($"UnloadSceneAsync - sceneName: {sceneName}", Instance);
            return AddressablesScenesManager.UnloadSceneAsync(sceneName);
        }
