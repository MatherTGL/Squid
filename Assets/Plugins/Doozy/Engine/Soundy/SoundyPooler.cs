...
namespace Doozy.Engine.Soundy
{
    ...
    public class SoundyPooler : MonoBehaviour
    {
        ...
        public static void ClearPool(bool keepMinimumNumberOfControllers = false)
        {
            if (keepMinimumNumberOfControllers)
            {
                ...
                if (Instance.DebugComponent) DDebug.Log(
                    $"Clear Pool - Killed {killedControllersCount} Controllers - {Pool.Count}' Controllers Available", Instance);
            ...
            Pool.Clear();
            if (Instance.DebugComponent) DDebug.Log(
                $"Clear Pool - Killed All Controllers - {Pool.Count} Controllers Available", Instance);
        }
        ...
        public static void PopulatePool(int numberOfControllers)
        {
            ...
            if (Instance.DebugComponent) DDebug.Log(
                $"Populate Pool - Added {numberOfControllers} Controllers to the Pool - {Pool.Count} Controllers Available", Instance);
        }
        ...
        public static void PutControllerInPool(SoundyController controller)
        {
            ...
            if (Instance.DebugComponent) DDebug.Log(
                $"Put '{controller.name}' Controller in the Pool - {Pool.Count} Controllers Available", Instance);
        }
