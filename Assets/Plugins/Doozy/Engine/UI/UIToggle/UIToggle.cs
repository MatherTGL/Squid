...
//            }
        ...
        {
            if (DebugComponent || debug) DDebug.Log(
                $"({name}) - [{(IsOn ? UIToggleState.On : UIToggleState.Off)}]{behavior.BehaviorType} - {action}.", this);
        }
        ...
        private IEnumerator ExecuteToggleBehaviorEnumerator(UIToggleBehavior behavior)
        {
            ...
            switch (behavior.BehaviorType)
            {
                case UIToggleBehaviorType.OnClick:
                    if (DeselectButtonAfterClick)
                    {
                        if (DebugComponent) DDebug.Log(
                            $"({name}) - [{(IsOn ? UIToggleState.On : UIToggleState.Off)}]{behavior.BehaviorType} - Deselect Button.", this);
                        DeselectToggle();
                    }
                ...
                case UIToggleBehaviorType.OnPointerEnter:
                    ...
                    {
                        if (DebugComponent) DDebug.Log(
                            $"({name}) - [{(IsOn ? UIToggleState.On : UIToggleState.Off)}]{behavior.BehaviorType} - Select Button.", this);
                        SelectToggle();
                    }
