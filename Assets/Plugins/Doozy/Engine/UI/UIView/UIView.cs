...
namespace Doozy.Engine.UI
{
	...
	public class UIView : UIComponentBase<UIView>
	{
		...
		private bool m_withoutCanvas => Canvas == null;
...
//            if (HasChildUIViews && DisableGameObjectWhenHidden) DisableGameObjectWhenHidden = false;
		...
		public void CancelAutoHide()
		{
			...
			Coroutiner.Stop(m_autoHideCoroutine);
		...
		public void Hide(bool instantAction = false)
		{
			...
			{
				DDebug.Log(
					$"You are trying to HIDE the ({ViewCategory})({ViewName}) UIView, but you did not enable any HIDE animations. Enable at least one HIDE animation in order to fix this issue.", this);
			...
			m_hideCoroutine = Coroutiner.Start(HideEnumerator(instantAction));
		}
		...
		{
			m_autoHideCoroutine = Coroutiner.Start(HideWithDelayEnumerator(delay));
		}
		...
		public void InstantShow()
		{
			...
			if (HasChildUIViews) Coroutiner.Start(ShowViewNextFrame(ViewCategory, ViewName, true));
		}
		...
		public void Show(bool instantAction = false)
		{
			...
			{
				DDebug.Log(
					$"You are trying to SHOW the ({ViewCategory})({ViewName}) UIView, but you did not enable any SHOW animations. Enable at least one SHOW animation in order to fix this issue.", this);
			...
			m_showCoroutine = Coroutiner.Start(ShowEnumerator(instantAction));
			if (HasChildUIViews) Coroutiner.Start(ShowViewNextFrame(ViewCategory, ViewName, instantAction));
		}
...
//            RemoveNullChildUIButtons();
				...
				case UIViewStartBehavior.PlayShowAnimation:
					...
					if (Settings.UseOrientationDetector)
					{
						if (OrientationDetector.CurrentOrientation == DetectedOrientation.Unknown)
							Coroutiner.Start(ExecuteGetOrientationEnumerator());
						else
					...
					{
						Show();
						if (HasChildUIViews) Coroutiner.Start(ShowViewNextFrame(ViewCategory, ViewName, false));
					}
		...
		private void StopHide()
		{
			...
			Coroutiner.Stop(m_hideCoroutine);
		...
		private void StopShow()
		{
			...
			Coroutiner.Stop(m_showCoroutine);
...
//            UpdateChildUIButtonsStartValues();
			...
			if (Settings.AutoDisableUIInteractions)
			{
...
//                    yield return new WaitForSecondsRealtime(HideBehavior.Animation.TotalDuration + UIViewSettings.DISABLE_WHEN_HIDDEN_TIME_BUFFER); //wait for seconds realtime (ignore Unity's Time.Timescale)
		...
		public static UIView GetView(string viewCategory, string viewName)
		{
			var views = new List<UIView>(); //create temp list
			foreach (UIView view in Database)
			{
				if (!view.ViewCategory.Equals(viewCategory)) continue;
				if (!view.ViewName.Equals(viewName)) continue;
				return view; //categories and names match -> add to list
			}
			Debug.LogError("No view found with category: " + viewCategory + " and name: " + viewName);
		...
		private static void ExecuteHide(string viewCategory, string viewName, bool instantAction = false)
		{
			...
			foreach (UIView view in Database)
			{
				...
				if (DoozySettings.Instance.DebugUIView) DDebug.Log(
					$"Hiding the ({viewCategory})({viewName}) UIView. This is {(instantAction ? "" : "NOT ")}an instant action.");
		...
		private static void ExecuteHideCategory(string viewCategory, bool instantAction = false)
		{
			...
			foreach (UIView view in Database)
			{
				...
				if (!view.gameObject.activeInHierarchy) continue;
				if (DoozySettings.Instance.DebugUIView) DDebug.Log(
					$"Hiding the ({viewCategory})({view.ViewName}) UIView. This is {(instantAction ? "" : "NOT ")}an instant action.");
				view.Hide(instantAction); //hide the UIView
			}
		...
		private static void ExecuteShow(string viewCategory, string viewName, bool instantAction)
		{
			...
			foreach (UIView view in Database)
			{
				...
				{
					if (DoozySettings.Instance.DebugUIView) DDebug.Log(
						$"Showing the ({viewCategory})({viewName}) UIView. This is {(instantAction ? "" : "NOT ")}an instant action.");
						...
						switch (view.TargetOrientation)
						{
							case TargetOrientation.Portrait:
								if (DoozySettings.Instance.DebugUIView)
									DDebug.Log(
										$"Showing the ({viewCategory})({viewName}) UIView. This is {(instantAction ? "" : "not ")}an instant action.{(DoozySettings.Instance.UseOrientationDetector ? $" TargetOrientation: {TargetOrientation.Portrait}" : "")}");
							...
							case TargetOrientation.Any:
								if (DoozySettings.Instance.DebugUIView)
									DDebug.Log(
										$"Showing the ({viewCategory})({viewName}) UIView. This is {(instantAction ? "" : "not ")}an instant action.{(DoozySettings.Instance.UseOrientationDetector ? $" TargetOrientation: {TargetOrientation.Any}" : "")}");
								view.Show(instantAction);
								break;
							default:
					...
					case DetectedOrientation.Landscape:
						switch (view.TargetOrientation)
						{
							...
							case TargetOrientation.Landscape:
								if (DoozySettings.Instance.DebugUIView)
									DDebug.Log(
										$"Showing the ({viewCategory})({viewName}) UIView. This is {(instantAction ? "" : "not ")}an instant action.{(DoozySettings.Instance.UseOrientationDetector ? $" TargetOrientation: {TargetOrientation.Landscape}" : "")}");
								view.Show(instantAction);
								break;
							case TargetOrientation.Any:
								if (DoozySettings.Instance.DebugUIView)
		...
		private static void ExecuteShowCategory(string viewCategory, bool instantAction = false)
		{
			...
			foreach (UIView view in Database)
			{
				...
				{
					if (DoozySettings.Instance.DebugUIView) DDebug.Log(
						$"Showing the ({viewCategory})({view.ViewName}) UIView. This is {(instantAction ? "" : "NOT ")}an instant action.");
					view.Show(instantAction); //show the UIView
					continue; //continue to the next Database entry
				}
					...
					case DetectedOrientation.Portrait:
						switch (view.TargetOrientation)
						{
							case TargetOrientation.Portrait:
								if (DoozySettings.Instance.DebugUIView)
									DDebug.Log(
										$"Showing the ({viewCategory})({view.ViewName}) UIView. This is {(instantAction ? "" : "not ")}an instant action.{(DoozySettings.Instance.UseOrientationDetector ? $" TargetOrientation: {TargetOrientation.Portrait}" : "")}");
								view.Show(instantAction);
								break;
							case TargetOrientation.Landscape:
								view.Visibility = VisibilityState.Visible;
							...
							case TargetOrientation.Any:
								if (DoozySettings.Instance.DebugUIView)
									DDebug.Log(
										$"Showing the ({viewCategory})({view.ViewName}) UIView. This is {(instantAction ? "" : "not ")}an instant action.{(DoozySettings.Instance.UseOrientationDetector ? $" TargetOrientation: {TargetOrientation.Any}" : "")}");
								view.Show(instantAction);
								break;
							default:
								throw new ArgumentOutOfRangeException();
					...
					case DetectedOrientation.Landscape:
						switch (view.TargetOrientation)
						{
							...
							case TargetOrientation.Landscape:
								if (DoozySettings.Instance.DebugUIView)
									DDebug.Log(
										$"Showing the ({viewCategory})({view.ViewName}) UIView. This is {(instantAction ? "" : "not ")}an instant action.{(DoozySettings.Instance.UseOrientationDetector ? $" TargetOrientation: {TargetOrientation.Landscape}" : "")}");
								view.Show(instantAction);
								break;
							case TargetOrientation.Any:
								if (DoozySettings.Instance.DebugUIView)
