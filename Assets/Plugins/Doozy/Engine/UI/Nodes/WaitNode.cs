...
namespace Doozy.Engine.UI.Nodes
{
    ...
    public class WaitNode : Node
    {
        ...
        public string WaitForInfoTitle
        {
            get
            {
                ...
                switch (WaitFor)
                {
                    case WaitType.Time:
                        return
                            $"{(RandomDuration ? $"[{DurationMin} - {DurationMax}]" : $"{Duration}")} {UILabels.Seconds}";
                    ...
                    case WaitType.ActiveSceneChange:
                        ...
                        switch (GetSceneBy)
                        {
                            case GetSceneBy.Name:
                                return $"{UILabels.Scene}: {(string.IsNullOrEmpty(SceneName) ? "---" : SceneName)}";
        ...
        {
            if (DebugMode) DDebug.Log(
                $"Active Scene Changed - Replaced Scene: {current.name} // Next Scene: {next.name}", this);
        ...
        private void OnUIViewMessage(UIViewMessage message)
        {
            ...
            if (DebugMode) DDebug.Log(
                $"UIViewMessage received: {message.Type} {message.View.ViewCategory} / {message.View.ViewName} // Listening for: {ViewCategory} / {ViewName}", this);
        ...
        private void OnUIButtonMessage(UIButtonMessage message)
        {
            ...
            if (DebugMode) DDebug.Log(
                $"UIButtonMessage received: {message.Type} {message.ButtonName} // Listening for: {ButtonName}", this);
        ...
        private void OnUIDrawerMessage(UIDrawerMessage message)
        {
            ...
            if (DebugMode) DDebug.Log(
                $"UIDrawerMessage received: {message.Type} {message.Drawer.DrawerName} // Listening for: {DrawerName}", this);
        ...
        public override void CheckForErrors()
        {
            ...
            switch (WaitFor)
            {
                ...
                case WaitType.SceneUnload:
                case WaitType.ActiveSceneChange:
                    ErrorNoSceneName = GetSceneBy is GetSceneBy.Name or GetSceneBy.Addressable && string.IsNullOrEmpty(SceneName.Trim());
