            ...
            switch (InputData.InputMode)
            {
                ...
                case InputMode.VirtualButton:
                    if (UnityEngine.Input.GetButtonDown(InputData.VirtualButtonName))
                    {
                        if (Actions != null)
                        {
                            if (DebugComponent) DDebug.Log(
                                $"Executed UIAction via Virtual Button: {InputData.VirtualButtonName}", gameObject);
                            execute = true;
                        }
                    }
                    else if (InputData.EnableAlternateInputs && UnityEngine.Input.GetButtonDown(InputData.VirtualButtonNameAlt))
                    {
                        ...
                        {
                            if (DebugComponent) DDebug.Log(
                                $"Executed UIAction via Virtual Button: {InputData.VirtualButtonNameAlt}", gameObject);
                            execute = true;
                        }
                    }
