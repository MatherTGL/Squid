...
namespace Doozy.Engine.UI
{
    ...
    public class UIDrawer : UIComponentBase<UIDrawer>, IDragHandler, IBeginDragHandler, IEndDragHandler
    {
        ...
        public override void Awake()
        {
            ...
            {
                DDebug.Log(
                    $"The '{DrawerName}' drawer does not have a container referenced. This is the main drawer component and should not be missing. Either reference it or delete this gameObject. For this session, this gameObject has been disabled. (HINT: You can create a new UIDrawer to see how the container should be referenced)",
                           gameObject);
        ...
        public void Close(bool instantAction = false, bool willTriggerBackButton = false)
        {
            if (Closed && !IsDragged) return;
            if (DebugComponent) DDebug.Log(
                $"'{DrawerName}' - Closed {(instantAction ? "in zero seconds!" : "with animation.")}", gameObject);
            InitiateClose();   //prepare the drawer to be closed
            willTriggerBackButtonOnClose = willTriggerBackButton;
        ...
        public void Open(bool instantAction = false)
        {
            ...
            if (DebugComponent) DDebug.Log(
                $"'{DrawerName}' - Opened {(instantAction ? "in zero seconds!" : "with animation.")}", gameObject);
        ...
        public void UpdateArrowContainer() { if(ArrowEnabled) Arrow.Container.Copy(Container.RectTransform); }
        ...
        private bool willTriggerBackButtonOnClose = false;
        ...
        private void FinalizeClose()
        {
            ...
            if (willTriggerBackButtonOnClose)
            {
                BackButton.Instance.Execute();
                willTriggerBackButtonOnClose = false;
            }
        }
        ...
        private void UpdateArrowActiveState() { if(ArrowEnabled) Arrow.Container.gameObject.SetActive(ArrowEnabled); }
        ...
        public static void Close(string drawerName, bool debug = false)
        {
            ...
            if (drawer == null)
            {
                if (debug) DDebug.LogError(
                    $"Unable to close the '{drawerName}' drawer because no such UIDrawer was found in the Database.");
                return;
            }

            if (AnyDrawerOpened && OpenedDrawer == drawer) OpenedDrawer = null;
            drawer.Close(false, true);
        }
        ...
        public static void Open(string drawerName, bool debug = false)
        {
            ...
            {
                if (debug) DDebug.LogError(
                    $"Unable to open the '{drawerName}' drawer because no such UIDrawer was found in the Database.");
                return;
            }
        ...
        public static void Toggle(string drawerName, bool debug = false)
        {
            ...
            {
                if (debug) DDebug.LogError(
                    $"Unable to toggle the '{drawerName}' drawer because no such UIDrawer was found in the Database.");
                return;
            }
